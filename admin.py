...
.admin-header h1 { color: #f0f0f0; margin: 0; font-size: 1.6rem; }
.admin-header p  { color: #90a4ae; margin: 0.2rem 0 0; font-size: 0.85rem; }
.kpi-card {
    background: white;
...
.kpi-card .kpi-label { font-size: 0.8rem; color: #6c757d; margin-top: 0.2rem; }
.kpi-card .kpi-delta { font-size: 0.75rem; margin-top: 0.15rem; }
.escalation-badge { background: #f8d7da; color: #721c24; padding: 2px 8px; border-radius: 8px; font-size: 0.75rem; font-weight: 600; }
.resolved-badge   { background: #d4edda; color: #155724; padding: 2px 8px; border-radius: 8px; font-size: 0.75rem; font-weight: 600; }
.open-badge       { background: #cce5ff; color: #004085; padding: 2px 8px; border-radius: 8px; font-size: 0.75rem; font-weight: 600; }
...
st.markdown(
    ...
    <h1>🛡️ {COMPANY_NAME} Support Dashboard</h1>
    <p>Human Support Operations · Escalation Queue · Audit & Governance · Metrics</p>
    </div>""",
    unsafe_allow_html=True,
)
...
with tab_overview:
    ...
    def kpi(col, value, label, delta="", delta_color="normal"):
        col.markdown(
            ...
            <div class="kpi-delta" style="color:{'#28a745' if delta_color=='good' else '#dc3545' if delta_color=='bad' else '#6c757d'}">{delta}</div>
            </div>""",
            unsafe_allow_html=True,
        )
...
with tab_escalation:
    ...
    if not open_escalated:
        ...
        st.warning(f"⚠️ **{len(open_escalated)} escalated conversations** require human attention")

        for ticket in open_escalated:
            with st.expander(
                f"🎫 {ticket['ticket_id']} | {ticket['priority']} | {ticket['customer_name']} — {ticket['subject'][:60]}",
                expanded=ticket["priority"] in ("Critical", "High"),
            ):
                col_info, col_actions = st.columns([3, 1])
                with col_info:
                    st.markdown(f"**Customer:** {ticket['customer_name']} (`{ticket['customer_id']}`)")
                    st.markdown(f"**Priority:** {ticket['priority']}  |  **Category:** {ticket['category']}")
                    st.markdown(f"**Created:** {ticket['created_at'][:19].replace('T', ' ')} UTC")
                    st.markdown(f"**Session:** `{ticket.get('session_id', 'N/A')[:8]}…`")
                    st.markdown("**Description:**")
                    st.text(ticket["description"][:600])

                    if ticket.get("notes"):
                        st.markdown("**Agent Notes:**")
                        for note in ticket["notes"]:
                            st.info(f"{note['timestamp'][:16]}: {note['text']}")

                with col_actions:
                    st.markdown("**Actions**")
                    new_note = st.text_area("Add note", key=f"note_{ticket['ticket_id']}", height=80)
                    ...
                    new_status = st.selectbox(
                        ...
                        index=["Open", "In Progress", "Pending Customer", "Resolved", "Closed"].index(
                            ticket["status"]
                        ),
                        key=f"status_{ticket['ticket_id']}",
                    )
                    if st.button("✅ Update", key=f"upd_{ticket['ticket_id']}"):
